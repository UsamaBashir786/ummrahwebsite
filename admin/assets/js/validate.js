...
if (!window.returnStopValidationInitialized) {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const addButtons = document.querySelectorAll('#add-return-stop');
    if (addButtons.length > 1) {
      console.error(`Found ${addButtons.length} elements with id="add-return-stop". IDs must be unique.`);
    }
    ...
    function addReturnStopHandler() {
      console.log('Adding new return stop'); // Debug: Should appear once per click
      const newRow = document.createElement('div');
      newRow.className = 'return-stop-row row mb-3';
      newRow.innerHTML = `
        ...
        <div class="col-md-6">
          ...
          <div class="input-group">
            ...
            <button type="button" class="btn btn-sm btn-danger" onclick="this.closest('.return-stop-row').remove()">Remove</button>
          </div>
          <div class="invalid-feedback"></div>
        </div>
      `;
      returnStopsContainer.insertBefore(newRow, addReturnStopButton.parentElement);
    ...
    addReturnStopButton.removeEventListener('click', addReturnStopHandler);
